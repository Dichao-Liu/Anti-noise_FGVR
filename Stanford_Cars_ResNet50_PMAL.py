...
def train(nb_epoch, batch_size, store_name, num_class=0, start_epoch=0, data_path=''):
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        if epoch < 50 or epoch >= 100:
            ...
            torch.save(decoder1, './' + store_name + '/decoder1_{}.pth'.format(epoch))
            torch.save(decoder2, './' + store_name + '/decoder2_{}.pth'.format(epoch))
            torch.save(decoder3, './' + store_name + '/decoder3_{}.pth'.format(epoch))
            net.to(device)
            decoder1.to(device)
            decoder2.to(device)
            decoder3.to(device)
