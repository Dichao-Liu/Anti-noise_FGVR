...
def train(nb_epoch, batch_size, store_name, num_class=1, start_epoch=0, data_path=''):
    ...
    if not args.from_local:
        net_teacher = torch.load("weightsFromCloud/Stanford_Cars_TResNet-L_Teacher_Network.pth")
    else:
        net_teacher = torch.load('results/Stanford_Cars_TResNet_L_PMAL/model.pth')
...
if __name__ == '__main__':
    ...
    train(nb_epoch=200,  # number of epoch
          ...
          store_name='results/Stanford_Cars_TResNet_L_Distillation',  # the folder for saving results
